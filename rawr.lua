...
getgenv()['Check_Setting'] = function(Name)
    if not _G.Dis and not isfolder('SioZeng Hub BF Premium') then
        makefolder('SioZeng Hub BF Premium')
    end
    if not _G.Dis and not isfile('SioZeng Hub BF Premium/'..Name..'.json') then
        writefile('SioZeng Hub BF Premium/'..Name..'.json',JsonEncode(_G.Setting_table))
    end
end
getgenv()['Get_Setting'] = function(Name)
    if not _G.Dis and isfolder('SioZeng Hub BF Premium') and isfile('SioZeng Hub BF Premium/'..Name..'.json') then
        _G.Setting_table = JsonDecode(readfile('SioZeng Hub BF Premium/'..Name..'.json'))
...
if IKAI then
	do
		local ui = game.CoreGui:FindFirstChild("Siozeng")
		if ui then
			ui:Destroy()
		end
	end
	...
	function library:Window(text,text2,text3,logo,keybind)
		...
		local Siozeng = Instance.new("ScreenGui")
		Siozeng.Name = "Siozeng"
		Siozeng.Parent = game.CoreGui
		Siozeng.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
